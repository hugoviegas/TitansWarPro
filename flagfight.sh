flagfight_fight() {
  ...
  until [ -s "BREAK_LOOP" ]; do
    ...
    if awk -v ush="$(cat USH)" -v hlhp="$(cat HLHP)" 'BEGIN { exit !(ush < hlhp) }' && [ "$(($(date +%s) - $(cat last_heal)))" -gt 90 -a "$(($(date +%s) - $(cat last_heal)))" -lt 300 ]; then
      (
  w3mc -cookie -o http_proxy=$PROXY -o accept_encoding=UTF-8 -debug -dump_source "${URL}$(cat SHIELD)" -o user_agent="$(shuf -n1 "$TMP"/userAgent.txt)" >$src_ram
      ) </dev/null &>/dev/null &
      time_exit 17
      cf_access
      cat USH >$full_ram
...
flagfight_start() {
  ...
  w3mc -cookie -o http_proxy=$PROXY -o accept_encoding=UTF-8 -debug -dump_source "$URL/train" -o user_agent="$(shuf -n1 "$TMP"/userAgent.txt)" | grep -o -E '\(([0-9]+)\)' | sed 's/[()]//g' >$full_ram
    ...
    (
  w3mc -cookie -o http_proxy=$PROXY -o accept_encoding=UTF-8 -debug -dump_source "$URL/flagfight/enterFight" -o user_agent="$(shuf -n1 "$TMP"/userAgent.txt)" >$src_ram
    ) </dev/null &>/dev/null &
    time_exit 17
    #printf "\nFlag fight\n$URL\n"
