...
curl -s -L https://raw.githubusercontent.com/sharesourcecode/TitansWarMacro/refs/heads/master/LANGUAGE.po > $HOME/twm/LANGUAGE.po
. ~/twm/info.lib
colors
...
if [ ! -z "$RUN" ] ; then
 ...
 '
  ...
  if ! curl -s --head --request GET titanswar.net|grep "200 OK" > /dev/null ; then
   printf "${WHITEb_BLACK}$(G_T "Network error")! $(G_T "Please check your internet connection").${BLACK_RESET}\n"
   exit 1
  else
