...
# Process user input for server selection
		if [ "$UR" = $'\0' ]; then
			echo "1" >"${SHARE_DIR}/ur_file"  # Defaults to server 1(Brazilian) if Enter is pressed
		elif [ "$UR" = '0' ]; then
			echo "10" >"${SHARE_DIR}/ur_file"  # Select Russian server if '(1)0' is pressed
		elif [ "$UR" = '1' ]; then
				echo "11" >"${SHARE_DIR}/ur_file"  # Select Croatian server if '(1)1' is pressed
		elif [ "$UR" = '2' ]; then
			echo "12" >"${SHARE_DIR}/ur_file"  # Select Chinese server if '(1)2' is pressed
		elif [ "$UR" = '3' ]; then
			echo "13" >"${SHARE_DIR}/ur_file"  # Select English server if '(1)3' is pressed
		elif [ "$UR" = $'\177' ]; then
...
# Check if ur_file exists and is not empty; if so, read from it; otherwise, show the first menu.
	if [ -f "${SHARE_DIR}/ur_file" ] && [ -s "${SHARE_DIR}/ur_file" ]; then
		UR="$(cat "${SHARE_DIR}/ur_file")"
	else
		menu_one
	fi
...
if [ -f "${SHARE_DIR}/fileAgent.txt" ] && [ -s "${SHARE_DIR}/fileAgent.txt" ]; then
    UA=$(cat "${SHARE_DIR}/fileAgent.txt") # Read existing user agent
else
    printf_t "Set up User-Agent: " "$COLOR_RESET" " [1 to 2]"
...
# Handle user agent selection based on input
		...
		(0)
			clear
			echo "0" >"${SHARE_DIR}/fileAgent.txt" # Save selection to file
