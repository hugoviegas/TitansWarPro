...
#for pattern in "$HOME/twm/allies.lib"; do
...
#grep -oE '\$\(G_T "[^"]+' twm/allies.lib | sed 's/^\$(G_T "//'
grep -oP '\$\(G_T "\K[^"]+' $SCRIPT >TEXT
...
if grep -q -o -E "\|$TRANSLATE\|" "$HOME/twm/LANGUAGE.po"; then
	echo 'This term is already translated'
	exit 0
fi
#echo 'de Deutsch|en English|es Español|fr Français|hi hindi|id Indonesian|it Italiano|pl Polski|pt Português|ro Română|ru Русский|sr Srpski|zh 中文' > $HOME/twm/LANGUAGE.po
